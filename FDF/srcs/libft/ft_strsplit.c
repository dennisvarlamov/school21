...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/18 06:59:52 by nglynis           #+#    #+#             */
/*   Updated: 2020/01/24 01:01:11 by jabilbo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_free(char **words, size_t i)
{
	while (i--)
		ft_strdel(&(words[i]));
	free(*words);
...
char			**ft_strsplit(char const *s, char c)
{
	...
	if (!s || !(line = ft_strdup((char *)s)))
		return (NULL);
	words = ft_get_words(line, c, ft_count(line, c));
	free(line);
	return (words);
}
